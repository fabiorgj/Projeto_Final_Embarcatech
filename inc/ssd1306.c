...
void ssd1306_draw_char(ssd1306_t *ssd, char c, uint8_t x, uint8_t y)
{
  uint16_t index = 0;
  char ver=c;
  if (c >= 'A' && c <= 'Z') // Comparação usando valores ASCII
  {
    index = (c - 'A' + 11) * 8; // Para letras maiúsculas
  } else if (c >= '0' && c <= '9') // Comparação usando valores ASCII
  {
    index = (c - '0' + 1) * 8; // Adiciona o deslocamento necessário
  } else if (c >= 'a' && c <= 'z') // Comparação usando valores ASCII para letras minúsculas
  {
    index = (c - 'a' + 37) * 8; // Adiciona o deslocamento necessário para letras minúsculas
  }else  if (c == ':'){
    index = 63 * 8; // Para letras maiúsculas
  }else  if (c == '.'){
    index = 64 * 8; // Para letras maiúsculas
  }else  if (c == '*'){
    index = 65 * 8; // Para letras maiúsculas
  }
